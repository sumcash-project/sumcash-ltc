...
uint256 hashGenesisBlock("0x8207cc10210f8c1cc2c30d435e637330eb27576bcd126e823e95a81da8769008");
...
const string strMessageMagic = "Sumcash Signed Message:\n";
...
{
    // Sumcash: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 2314.81481 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 43200); // Sumcash: 43200 is 30 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // Sumcash: each block
static const int64 nTargetSpacing = 60; // Sumcash: 60 second blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sumcash: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sumcash: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xf2; // set for sumcash
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce   = 2088111234;

        if (fTestNet)
        {
            block.nTime    = 1641914067;
        ...
        assert(block.hashMerkleRoot == uint256("0xa14add4166fd2eb3c8f52566d23b68438094f4a72e359ad7f92506f0f813ea2f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xba, 0xd2 }; // Sumcash set
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SumcashMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SumcashMiner(CWallet *pwallet)
{
    printf("SumcashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sumcash-miner");
    ...
    try { loop {
        ...
        printf("Running SumcashMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SumcashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SumcashMiner, pwallet));
}
